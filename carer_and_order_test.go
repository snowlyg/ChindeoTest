...
func TestCarrListNoPageSuccess(t *testing.T) {
	...
	obj.Value("code").Equal(200)
	obj.Value("message").String().Equal("请求成功")
	obj.Value("data").Array().First().Object().Value("id").Equal(Carer.ID)
}
...
func TestCarrShowSuccess(t *testing.T) {
	...
	obj := e.GET("/care/v1/inner/carer/{id}", Carer.ID).
		WithHeaders(map[string]string{"X-Token": Token, "AuthType": strconv.FormatInt(int64(common.AUTH_TYPE_SERVER), 10)}).
		WithCookie("PHPSESSID", PHPSESSID).
		Expect().
		Status(http.StatusOK).JSON().Object()
...
func TestCarrShowAfterOrderAddSuccess(t *testing.T) {
	...
	carerTimeTypeText = common.GetS(obj.Value("data").Object().Value("time_type").Raw())
}
...
func TestCarrOrderCancelPayCarerSuccess(t *testing.T) {
	...
	obj := e.GET("/care/v1/inner/order/cancel/{id}", CarerOrder.ID).
		WithHeaders(map[string]string{"X-Token": Token, "AuthType": strconv.FormatInt(int64(common.AUTH_TYPE_SERVER), 10)}).
		WithCookie("PHPSESSID", PHPSESSID).
		Expect().
		Status(http.StatusOK).JSON().Object()
...
func TestCarrOrderShowReturnCarerSuccess(t *testing.T) {
	...
	obj := e.GET("/care/v1/inner/order/{id}", CarerOrder.ID).
		WithHeaders(map[string]string{"X-Token": Token, "AuthType": strconv.FormatInt(int64(common.AUTH_TYPE_SERVER), 10)}).
		WithCookie("PHPSESSID", PHPSESSID).
		Expect().
		Status(http.StatusOK).JSON().Object()
	...
	orderReturn.Value("total").Equal(common.Ftos(CarerOrder.Total))
